  ...
  return axios
    ...
    .then(({members: buixieval}) =>
      members.map((member) => {
        ...
        const buixievalMember = buixieval.find((b) => parseInt(b.f_id, 10) === member.id);
...
const winningTeamColor = (members) => {
  const contributed = (members) =>
    members.reduce(
      (total, member) => total + (member.buixieval ? member.buixieval.contributed : 0),
      0.0
    );

  const team = (members, name) =>
    members.filter((member) => member.buixieval && member.buixieval.team === name);
