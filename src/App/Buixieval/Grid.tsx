...
const Grid = ({width, height, backers, selectMember}: any) => {
  if (backers.length === 0) {
  ...
  const totalContributed = backers.reduce(
    (sum: any, backer: any) => sum + backer.buixieval.contributed,
    0
  );
  ...
  const split = () => {
    ...
    let first = backers
      .slice(0, 1)
      .reduce((sum: any, backer: any) => sum + backer.buixieval.contributed, 0);
  ...
  const partition1 =
    sliced[0].reduce((sum: any, backer: any) => sum + backer.buixieval.contributed, 0) /
    totalContributed;
